...
def test_request(xml, register_id, user_id):
    ...
    # db updates
    db_res = update_db(testUtil, register_id, user_id, reciept_num, response, request)
    if db_res == 1:
          return response
    elif db_res == 0:
        return 'Error: You already tryed to fiscalize this reciept!'
    elif db_res == -1:
...
def update_db(testUtil, user_id, register_id, reciept, response, request):
    ...
    (test_id, done) = test_result_resolve(user_id, register_id, reciept)
